...
def handleInput():
    if input("Would you like to generate a new input file? y/n\n") == "y":
        ...
        jobs = []
        for index in range(1, num_of_jobs ):
            j = []
            j.append(index)
        ...
        for index, line in enumerate(inpt):
            if index == 0:
                ...
                print("The number of machines loaded : ",  num_of_machines   , "\n")
            else:
...
out_file = open("output_"+str(NUM_OF_CHROMOZOMS)+"chromozoms_"+str(NUM_OF_GEN)+"generations_"+str(num_of_machines)+"machines_"+str(num_of_jobs)+"jobs_"+str(file_times)+".txt", "w")
...
def mutate(chromosome:list):
    ...
    for i in range(len(indices_to_mutate)):
        ...
        legal = False
        while not legal:
            machine_rand = randint(0, num_of_machines)
            ...
            chromosome[0][indices_to_mutate[i]] = randint(0,num_of_machines)
            legal = True
            printMachineStatOut()
    span = makeSpan()
    chromosome[1] = span
...
def printMachineStatOut():
    ...
    for machine in machines_list:
        ...
        print("\n", file=out_file)
    print("Min makespan is : ",min(machine.span for machine in machines_list),file = out_file)
    print("Total sum of jobs' processing time : ", makeSpan(), file=out_file)
